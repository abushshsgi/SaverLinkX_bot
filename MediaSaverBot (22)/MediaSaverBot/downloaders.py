...
def download_tiktok_video(url):
    ...
    try:
        ...
        # Set up yt-dlp options for TikTok
        ydl_opts = {
            ...
            'verbose': True,
            'geo_bypass': True,
            'extractor_args': {'tiktok': {'bypass_api': True}},
        ...
        logger.error(f"yt-dlp TikTok download error: {str(e)}")
        error_msg = str(e).lower()
        if "private" in error_msg:
            raise DownloadError("This TikTok account is private. You can only download public content.")
        elif "not found" in error_msg or "does not exist" in error_msg:
            raise DownloadError("The TikTok video does not exist or has been removed.")
        else:
