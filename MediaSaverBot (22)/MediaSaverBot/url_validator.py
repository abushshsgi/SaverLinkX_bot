...
def is_tiktok_url(url):
    ...
    tiktok_patterns = [
        ...
        # Short TikTok URLs
        r'(https?://)?(vm\.)?(tiktok\.com|vt\.tiktok\.com)/([a-zA-Z0-9_.-]+)(/)?(\?.*)?$',
        # More specific vt.tiktok.com format
        r'(https?://)?vt\.tiktok\.com/([a-zA-Z0-9_.-]+)(/)?(\?.*)?$',
        # TikTok Trends
        r'(https?://)?(www\.|m\.)?tiktok\.com/t/([a-zA-Z0-9_.-]+)(/)?(\?.*)?$',
        # TikTok mobile app URLs
        r'(https?://)?(www\.|m\.)?tiktok\.com/v/(\d+)(\?.*)?$'
    ]
    
    for pattern in tiktok_patterns:
        if re.match(pattern, url):
            logger.info(f"Valid TikTok URL detected: {url}")
            return True
    
    logger.info(f"Not a valid TikTok URL: {url}")
    return False
...
def get_url_type(url):
    """
    Determine the type of URL (YouTube, Instagram, TikTok, Facebook, Twitter or invalid).
    Returns 'youtube', 'instagram', 'tiktok', 'facebook', 'twitter', or None.
    """
